    ...
    <section id="experience" className="container py-24 sm:py-32">
      ...
      <Accordion type="single" collapsible className="w-full md:w-3/4 mx-auto">
        {experienceData.map(exp => (
          <AccordionItem key={exp.value} value={exp.value} className="bg-background/50 px-4 rounded-lg mb-2">
            <AccordionTrigger>
              <div className="flex justify-between w-full pr-4 text-left">
                <span className="font-bold">{exp.role} @ {exp.company}</span>
                <span className="text-muted-foreground hidden sm:block">{exp.period}</span>
              </div>
            </AccordionTrigger>
            <AccordionContent>
              {exp.description}
